...
def render_inbox_page(inbox_ctrl: InboxController, audit_ctrl: AuditController, auth_manager: AuthManager):
    ...
    with ui.row().classes('w-full justify-center mb-4 transition-all') as queue_banner:
        queue_badge = ui.badge(t('inbox.queue_empty'), color='gray').classes('text-sm px-4 py-1')

    async def process_action_queue():
        q_len = len(state['action_queue'])
        if q_len > 0:
            queue_badge.set_text(t('inbox.queue_count', count=q_len))
            queue_badge.props('color="amber"')
        else:
            queue_badge.set_text(t('inbox.queue_empty'))
        ...
        try:
            while state['action_queue']:
                ...
                try:
                    if atype == 'archive':
                        ui.notify(t('inbox.archive_saving', filename=fname), type='info')
                        ...
                        if success:
                            ui.notify(t('inbox.archive_done', filename=fname), type='positive')
                            await auth_manager.execute(inbox_ctrl.delete_file, auth_manager.get_current_context(), u_login, folder, fname)
                        else:
                            ui.notify(t('inbox.archive_error', filename=fname), type='negative')

                    elif atype == 'delete':
                        u_login = auth_manager.get_current_context().user_login if action.get('is_personal') else None
                        audit_ctrl.add_audit_entry(
                            auth_manager.get_current_context(), EventDomain.FILE, LogLevel.INFO,
                            EventType.DELETE, 'Видалення файлу', None, {'file': str(fname), 'folder': str(folder)},
                        )
                        await auth_manager.execute(inbox_ctrl.delete_file, auth_manager.get_current_context(), u_login, folder, fname)
                        ui.notify(t('inbox.delete_done', filename=fname), type='positive')
                        ...
                        audit_ctrl.add_audit_entry(
                            auth_manager.get_current_context(), EventDomain.FILE, LogLevel.INFO,
                            EventType.UPDATE, 'Переназначення файлу', None,
                            {'file': str(fname), 'target': str(target), 'folder': str(folder)},
                        )
                        await auth_manager.execute(
                            inbox_ctrl.assign_file, auth_manager.get_current_context(),
                            fname, is_pers, target, f_folder,
                        )
                        ui.notify(t('inbox.assign_done', filename=fname, target=target), type='positive')

                except Exception as ex:
                    ui.notify(t('inbox.action_error', filename=fname, error=ex), type='negative')
    ...
    def add_to_queue(action_type: str, folder: str, filename: str, **kwargs):
        state['action_queue'].append({'type': action_type, 'filename': filename, 'folder': folder, **kwargs})
        ui.notify(t('inbox.added_to_queue', filename=filename), type='info')
    ...
    async def handle_upload(e: events.UploadEventArguments):
        try:
            ...
            audit_ctrl.add_audit_entry(
                auth_manager.get_current_context(), EventDomain.FILE, LogLevel.INFO,
                EventType.UPDATE, 'Завантаження файлу в inbox', None, {'file': filename},
            )
            await auth_manager.execute(inbox_ctrl.upload_root_file, auth_manager.get_current_context(), filename, file_data)
            ui.notify(t('inbox.upload_done', filename=filename), type='positive')
            e.sender.reset()
            await load_data()
        except ValueError as ex:
            ui.notify(t('inbox.upload_rejected', error=ex), type='warning')
        except Exception as ex:
            ui.notify(t('inbox.upload_error', error=ex), type='negative')
    ...
    async def load_data():
        try:
            ...
            ui.notify(t('inbox.load_error', error=e), type='negative')
    ...
    async def load_file_content(filename: str, f_type: str):
        try:
            ...
            state['file_content'] = t('inbox.content_error', error=e)
    ...
    async def download_file(filename: str, f_type: str):
        try:
            ...
            if file_buffer:
                ...
                ui.notify(t('inbox.download_started', filename=filename), type='positive')
            else:
                ui.notify(t('inbox.download_failed'), type='negative')
        except Exception as e:
            ui.notify(t('inbox.download_error', error=e), type='negative')

    async def confirm_and_queue_delete(folder: str, filename: str, is_personal: bool):
        result = await confirm_delete_dialog(t('inbox.confirm_delete', filename=filename))
        if result:
    ...
    def render_left_panel():
        with ui.column().classes('w-full h-full p-2 gap-4'):
            with ui.card().classes('w-full p-2 shadow-sm border border-blue-200 bg-blue-50/50 mb-2'):
                ui.label(t('inbox.upload_section')).classes('font-bold text-blue-800 text-sm mb-1')
            ...
            with ui.expansion(t('inbox.personal_inbox', count=len(state['personal_files'])), value=True) \
                    .props('header-class="font-bold text-red-800 bg-red-50 rounded" dense').classes('w-full'):
                with ui.column().classes('w-full gap-2 mt-2'):
                    if not state['personal_files']:
                        ui.label(t('inbox.no_files')).classes('text-xs text-gray-400 italic pl-1')
                    else:
                        for f in state['personal_files']:
                            is_sel = (state['selected_file'] == f and state['selected_type'] == 'inbox_personal')
                            in_queue = is_queued(f)
                            bg = 'bg-amber-50 border-amber-300' if in_queue else ('bg-red-100 border-red-300' if is_sel else 'bg-white hover:bg-gray-100 border-gray-200')
                            ...
                            with ui.row().classes(f'w-full p-2 border rounded cursor-pointer items-center gap-2 flex-nowrap transition-colors {bg}') \
            ...
            with ui.expansion(t('inbox.personal_outbox', count=len(state['outbox_files'])), value=True) \
                ...
                with ui.column().classes('w-full gap-2 mt-2'):
                    if not state['outbox_files']:
                        ...
                        for f in state['outbox_files']:
                            ...
                            in_queue = is_queued(f)
                            bg = 'bg-amber-50 border-amber-300' if in_queue else ('bg-green-100 border-green-300' if is_sel else 'bg-white hover:bg-gray-100 border-gray-200')
            ...
            with ui.expansion(t('inbox.shared_files', count=len(state['root_files'])), value=True) \
                ...
                with ui.column().classes('w-full gap-2 mt-2'):
                    if not state['root_files']:
                        ...
                        for f in state['root_files']:
                            ...
                            in_queue = is_queued(f)
                            bg = 'bg-amber-50 border-amber-300' if in_queue else ('bg-blue-100 border-blue-300' if is_sel else 'bg-white hover:bg-gray-100 border-gray-200')
                            icon_name = 'hourglass_empty' if in_queue else 'description'
        ...
        with ui.column().classes('w-full h-full flex flex-col p-0 overflow-hidden'):
            if not state['selected_file']:
                with ui.column().classes('w-full h-full items-center justify-center bg-gray-50 flex-grow'):
                    ...
                    ui.label(t('inbox.select_doc')).classes('text-gray-500 text-lg mt-4 text-center px-4')
                return
            ...
            with ui.row().classes('w-full bg-gray-100 border-b border-gray-200 p-2 sm:p-3 items-center justify-between shrink-0 flex-wrap gap-2'):
                ...
                if in_queue:
                    with ui.row().classes('items-center gap-2 px-4 py-2 bg-amber-100 rounded border border-amber-300 shrink-0'):
                        ...
                        ui.label(t('inbox.in_queue')).classes('text-amber-800 font-bold text-sm')
                else:
                    with ui.row().classes('items-center gap-1 sm:gap-2 shrink-0 flex-wrap justify-end'):
                        if f_type == 'inbox_personal':
                            resp_btn(t('inbox.btn_download'), 'download', 'blue', lambda: download_file(f_name, f_type))
                            if can_assign:
                                sel_assign = ui.select(user_options, label=t('inbox.select_user')).props('dense outlined hide-bottom-space').classes('w-24 sm:w-32')
                                resp_btn(t('inbox.btn_assign'), 'switch_account', 'blue',
                                         lambda: add_to_queue('assign', config.INBOX_DIR_PATH, f_name, target=sel_assign.value, is_personal=True))
                            if can_edit:
                                resp_btn(t('inbox.btn_archive'), 'archive', 'blue', lambda: add_to_queue('archive', config.INBOX_DIR_PATH, f_name, is_personal=True))
                            resp_btn(t('inbox.btn_delete'), 'delete', 'red', lambda: confirm_and_queue_delete(config.INBOX_DIR_PATH, f_name, True))

                        elif f_type == 'inbox_shared':
                            resp_btn(t('inbox.btn_download'), 'download', 'blue', lambda: download_file(f_name, f_type))
                            resp_btn(t('inbox.btn_archive'), 'archive', 'blue', lambda: add_to_queue('archive', config.INBOX_DIR_PATH, f_name))
                            if can_assign:
                                sel_assign = ui.select(user_options, label=t('inbox.select_user_shared')).props('dense outlined hide-bottom-space').classes('w-24 sm:w-32')
                                resp_btn(t('inbox.btn_assign'), 'switch_account', 'blue',
                                         lambda: add_to_queue('assign', config.INBOX_DIR_PATH, f_name, target=sel_assign.value, is_personal=False))
                            resp_btn(t('inbox.btn_delete'), 'delete', 'red', lambda: confirm_and_queue_delete(config.INBOX_DIR_PATH, f_name, False))
                            ...
                            resp_btn(t('inbox.btn_delete'), 'delete', 'red', lambda: confirm_and_queue_delete(config.OUTBOX_DIR_PATH, f_name, True))
            ...
            with ui.column().classes('w-full flex-grow p-0 m-0 bg-white relative overflow-hidden'):
                if state['is_loading_content']:
                    with ui.column().classes('absolute inset-0 items-center justify-center bg-white/80 z-10'):
                        ...
                        ui.label(t('inbox.reading')).classes('text-gray-500 mt-2')
    ...
    with ui.column().classes('lt-md w-full gap-3 px-2 pb-8 h-[calc(100vh-100px)] overflow-y-auto'):
        with ui.expansion(t('inbox.mobile_folders'), group='mobile_inbox') \
        ...
        with ui.expansion(t('inbox.mobile_content'), group='mobile_inbox') \
